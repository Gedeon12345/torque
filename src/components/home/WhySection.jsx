 ...
 role="list"
      ...
      >
        {advantages.map(({ title, description, icon: Icon }) => (
          <motion.li key={title} variants={fadeUp} className="bg-surface p-4 sm:p-5 lg:p-8">
            <span className="flex size-12 items-center justify-center rounded-xl border-[1.5px] border-accent text-accent">
              <Icon size={24} aria-hidden="true" />
            </span>
            <h3 className="mt-[18px] font-display text-[length:clamp(18px,5.4vw,23px)] font-bold leading-[1.1]">{title}</h3>
